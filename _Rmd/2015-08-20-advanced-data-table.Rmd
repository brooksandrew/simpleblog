...
data.table creators do favor `set` for [some things](http://stackoverflow.com/questions/16846380/how-to-apply-same-function-to-every-specified-column-in-a-data-table), like this task which can also be done w/ `lapply` and `.SD`.  I was actually directed to this solution after I posed [this question](http://stackoverflow.com/questions/31326691/apply-function-across-subset-of-columns-in-data-table-with-sdcols) on StackOverlow.  I was also pleased to learn that the 
...
dt[,mpg_lag1_bycyl:=shift(mpg, 1), keyby=cyl]


dt <- data.table(
  date=rep(seq(as.Date('2010-01-01'), as.Date('2015-01-01'), by='year'), 5), 
  ind=rpois(30, 5),
  entity=sort(rep(letters[1:5], 6))
  )
...
Unlike data.frame, the `:=` operator adds a column to both the object living in the global environment and used in the function.  I think this is because data.table shaves
computation time by not making copies unless explicity directed to.
...
So something like this, bypasses this behavior, but is likely somewhat less efficient (and elegant).  I suspect there's a cleaner and/or faster way to do this: keep some variables 
